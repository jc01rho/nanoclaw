...
function parseContent(raw: string): {
  ...
  try {
    ...
    return {
      ...
      skipWeekends: parsed.skipWeekends === true,
    ...
    return { prompt: raw, script: null, skipWeekends: false, originSessionId: null };
  }
}
...
function toOutput(session: ScopedSession, row: TaskRow) {
  ...
  return {
    ...
    skip_weekends: content.skipWeekends ? 1 : 0,
...
function createTask(args: Record<string, unknown>, ctx: CallerContext) {
  ...
  const prepared = prepareScheduledTask({
    ...
    script,
    skipWeekends: bool(args.skip_weekends),
...
function updateTaskCommand(args: Record<string, unknown>, ctx: CallerContext) {
  ...
  let existingContent: ReturnType<typeof parseContent> | undefined;
  const loadExistingContent = (): ReturnType<typeof parseContent> | undefined => {
    if (existingContent) return existingContent;
    for (const session of selectedSessions(args, ctx)) {
      const row = withInbound(session, (db) => selectTask(db, id));
      if (row) {
        existingContent = parseContent(row.content);
  ...
  const hasSkipWeekends = typeof args.skip_weekends === 'boolean' || typeof args.skip_weekends === 'string';
  if (hasSkipWeekends) {
    update.skipWeekends = bool(args.skip_weekends);
  }
  if (args.process_after !== undefined) {
    const parsedProcessAfter = parseProcessAfter(args.process_after);
    const skipWeekendsAfter = hasSkipWeekends
      ? update.skipWeekends === true
      : loadExistingContent()?.skipWeekends === true;
    update.processAfter = skipWeekendsAfter ? moveWeekendToNextWeekday(parsedProcessAfter) : parsedProcessAfter;
  }
  ...
  if (recurrence !== undefined) {
    ...
    if (script === undefined) {
      scriptAfter = loadExistingContent()?.script ?? null;
    }
...
registerResource({
  ...
  columns: [
    ...
    { name: 'script', type: 'string', description: 'Optional pre-task bash script.', updatable: true },
    {
      name: 'skip_weekends',
      type: 'boolean',
      description: 'Move weekend runs to the next weekday in the configured timezone.',
      updatable: true,
    },
  ],
  operations: {},
    ...
    create: {
      ...
      args: [
        ...
        },
        {
          name: 'skip_weekends',
          type: 'boolean',
          description: 'Skip Saturday/Sunday runs in the configured timezone.',
        },
        {
          name: 'group',
          type: 'string',
    ...
    update: {
      ...
      args: [
        ...
        { name: 'script', type: 'string', description: 'New pre-task script; "null"/"none" removes it.' },
        {
          name: 'skip_weekends',
          type: 'boolean',
          description: 'Enable or disable weekend skipping in the configured timezone.',
        },
        {
          name: 'group',
          type: 'string',
