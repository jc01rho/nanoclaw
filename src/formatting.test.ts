...
import { escapeXml, formatMessages, formatOutbound, stripReasoningBlocks, stripInternalTags } from './router.js';
import { parseTextStyles, parseSignalStyles } from './text-styles.js';

interface FormattingMessage {
...
function makeMsg(overrides: Partial<FormattingMessage> = {}): FormattingMessage {
  return {
    sender_name: 'Alice',
    content: 'hello',
...
describe('stripReasoningBlocks', () => {
  it('strips single-line think tags', () => {
    expect(stripReasoningBlocks('hello <think>secret</think> world')).toBe('hello  world');
  });

  it('strips multi-line think tags case-insensitively', () => {
    expect(stripReasoningBlocks('hello <THINK>\nsecret\nstuff\n</THINK> world')).toBe('hello  world');
  });
});
...
describe('formatOutbound', () => {
  ...
  it('strips reasoning blocks before returning text', () => {
    expect(formatOutbound('<think>secret</think>The answer is 42')).toBe('The answer is 42');
  });
});
