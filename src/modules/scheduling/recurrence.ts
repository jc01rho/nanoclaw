...
export async function handleRecurrence(inDb: Database.Database, session: Session): Promise<void> {
  ...
  for (const msg of recurring) {
    try {
      ...
      const cronNext = nextRunDate(interval, shouldSkipWeekends(msg.content));
...
function nextRunDate(interval: { next(): { toDate(): Date } }, skipWeekends: boolean): Date {
  for (let attempt = 0; attempt < MAX_RECURRENCE_ADVANCE_ATTEMPTS; attempt++) {
    const next = interval.next().toDate();
    if (!skipWeekends || !isWeekendInConfiguredTimezone(next)) return next;
  }
