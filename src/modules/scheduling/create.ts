...
export function moveWeekendToNextWeekday(processAfter: string): string {
...
export function prepareScheduledTask(input: {
  ...
  return {
    ...
    recurrence,
    script,
    processAfter: skipWeekends ? moveWeekendToNextWeekday(processAfter) : processAfter,
    skipWeekends,
  };
}
  ...
  const row = withInboundDb(agentGroupId, session.id, (db) => {
    insertTaskRow(db, {
      ...
      content: JSON.stringify({
        prompt: task.prompt,
        script: task.script,
        skipWeekends: task.skipWeekends === true,
