...
Agent workspaces. Each maps 1:1 to a `groups/<folder>/` directory containing `CLAUDE.md` and skills. Container config lives in `container_configs` (see §1.15 below); a `container.json` file is materialized at spawn time for the container runner to read.
