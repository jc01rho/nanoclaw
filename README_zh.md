...
`nanoclaw.sh` 会把您从一台全新机器一直带到一个可以直接发消息的命名智能体。它会在缺失时安装 Node、pnpm 和 Docker，把 Anthropic 兼容凭据写入本地 `.env`，构建智能体容器，并配对您的第一个渠道（Telegram、Discord、WhatsApp 或本地 CLI）。如果某一步失败，会自动调用 Claude Code 进行诊断并从中断处继续。
...
- **凭据配置** — 运行时凭据由宿主机 `.env` 中的 `ANTHROPIC_BASE_URL`、`ANTHROPIC_API_KEY`、`ANTHROPIC_AUTH_TOKEN` 等变量提供，并在容器启动时注入。
...
- `src/container-runner.ts` — 为每个智能体组启动容器并注入直接 LLM 调用所需 env
...
智能体运行在容器里，而不是躲在应用级权限检查之后。它们只能访问明确挂载的目录。凭据由宿主机 `.env` 管理，并在容器启动时以环境变量形式注入，用于直接访问配置好的 API 端点。您仍然应该审查自己要运行的代码，但代码库小到您真的能做到。完整的安全模型见 [安全文档](https://docs.nanoclaw.dev/concepts/security)。
