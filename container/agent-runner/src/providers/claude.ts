...
const PUBLIC_GUEST_K8S_ALLOWED_TOOLS = new Set([
  'Read',
  'Glob',
  'Grep',
  'WebSearch',
...
let activeRuntimePolicy: RuntimePolicy = 'default';

export function getToolAllowlist(policy: RuntimePolicy): string[] {
  return policy === 'public_guest_k8s' ? [...PUBLIC_GUEST_K8S_ALLOWED_TOOLS] : [...TOOL_ALLOWLIST, 'mcp__nanoclaw__*'];
}

export function isToolAllowedByRuntimePolicy(toolName: string, policy: RuntimePolicy): boolean {
  return policy !== 'public_guest_k8s' || PUBLIC_GUEST_K8S_ALLOWED_TOOLS.has(toolName);
}

function resolveRuntimePolicy(): RuntimePolicy {
  return activeRuntimePolicy === 'public_guest_k8s' || process.env.NANOCLAW_RUNTIME_POLICY === 'public_guest_k8s'
    ? 'public_guest_k8s'
    : 'default';
}
...
function archiveTranscriptFile(
  ...
  try {
    ...
    const filename = `${dateParts.year}-${dateParts.month}-${dateParts.day}-${name}.md`;
...
export class ClaudeProvider implements AgentProvider {
  ...
  constructor(options: ProviderOptions = {}) {
    ...
    activeRuntimePolicy = this.runtimePolicy;
    ...
    this.env = {
      ...
      NANOCLAW_RUNTIME_POLICY: this.runtimePolicy,
    };
  }
    ...
    const sdkResult = sdkQuery({
      ...
      options: {
        ...
        systemPrompt: instructions
          ...
          : undefined,
        allowedTools:
          this.runtimePolicy === 'public_guest_k8s'
            ? getToolAllowlist(this.runtimePolicy)
            : [...TOOL_ALLOWLIST, ...Object.keys(this.mcpServers).map(mcpAllowPattern)],
