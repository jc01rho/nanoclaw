...
Auth is handled by a host-managed token exposed to the runtime. The Vercel CLI still requires a token to be present, so pass the real token from configuration rather than relying on a proxy placeholder.
...
If this fails with an auth error, ask the user to add a Vercel token to host configuration such as `.env`, then retry `vercel whoami`.

## Deploying

Always use `--yes` to skip interactive prompts and pass a configured real token for auth.
