...
**Credential destination note:** Credentials are saved to `.env`. The current setup flow keeps them in host-managed env configuration, so the user does not need to do an extra credential migration step here.
