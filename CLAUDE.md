...
| `src/container-runner.ts` | Spawns per-agent-group Docker containers with session DB + outbox mounts and direct LLM env wiring |
...
API keys, OAuth tokens, endpoint URLs, and model names are configured from local `.env` and passed into per-agent containers at spawn time. The active Anthropic-compatible settings are `ANTHROPIC_BASE_URL`, `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `CLAUDE_CODE_OAUTH_TOKEN`, and `ANTHROPIC_MODEL`.

Credentialed external API calls now use direct host-managed `.env` configuration. Admin approvals in trunk are limited to DB-backed flows such as self-modification requests (`install_packages`, `add_mcp_server`) routed through `src/modules/approvals/primitive.ts` and `src/modules/approvals/response-handler.ts`.
